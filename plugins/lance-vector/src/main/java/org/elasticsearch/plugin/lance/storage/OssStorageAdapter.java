...
public class OssStorageAdapter {
    ...
    private static OssCredentials loadCredentials(String credentialsPath) throws IOException {
        ...
        if (Files.exists(path) == false) {
            throw new IOException(
                "OSS credentials file not found: "
                    + credentialsPath
                    + ". Please create a credentials file with access_key_id, access_key_secret, endpoint, and region."
            );
        }
    ...
    public InputStream readObject(String ossUri) throws IOException {
        ...
        String objectUrl = String.format("https://%s.%s/%s", uri.bucket, credentials.endpoint, uri.objectKey);
